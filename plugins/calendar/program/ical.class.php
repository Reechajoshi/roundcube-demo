 ...
 */
  function deleteComponent( $arg1, $arg2=FALSE  ) {
	...
	* This function get the attendees details array as argument and return the 
	* array containing email of attendees
	* Returns the array of attendees username, email and role 
	**/
	function getAttendeesComponent( $attendees_arr, &$attendees_email_arr, &$attendees_username_arr, &$attendees_role_arr ) {
		GLOBAL $RC_HELP;
		
		// initialise returning arrays
		$attendees_role_arr = $attendees_username_arr = $attendees_email_arr = array();
		
		for( $i = 0; $i < count( $attendees_arr ); $i++ )
		{
			// since the format is in MAILTO: <email>, retrive only email from it
			$attendee_email = str_ireplace( 'MAILTO:', '', $attendees_arr[ $i ][ 'value' ] );
			$attendee_details = $RC_HELP->get_user_det_by_email( $attendee_email );
			
			// for users who are not webmail users, derive username from exploding email
			if( empty( $attendee_details[ 'user_name' ] ) )
			{
				$explode_email = explode( "@", $attendee_email );
				$attendee_username = $explode_email[ 0 ];
			}
			else	
			{
				$attendee_username = $attendee_details[ 'user_name' ];
			}
			
			// retrive the role of users. if role not present, default is REQ_PARTICIPANT
			if( empty( $attendees_arr[ $i ][ 'params' ][ 'ROLE' ] ) )
			{
				$attendees_role = "REQ-PARTICIPANT|".$attendee_email;
			}
			else
			{
				$attendees_role = $attendees_arr[ $i ][ 'params' ][ 'ROLE' ]."|".$attendee_email;
			}
			
			$attendees_email_arr[] = $attendee_email;
			$attendees_username_arr[] = $attendee_username;
			$attendees_role_arr[] = $attendees_role;
 ...
 */
  function createAttendee() {
	...
	/**
	 *Macgregor Changes
	 *Creates the contents of the ics file for attendees
	**/
  
	// method to insert initial attendees content
	public function create_attendees( $invitees_email_str, $invitees_username_str, $invite_sent_str, $invitee_role_str )
	{
		$rcmail = rcmail::get_instance();
		$caldav_backend = new calendar_caldav($rcmail, 'database');
		$username = $rcmail->user->get_username();
		$invitees_email_array = explode( "|", $invitees_email_str );
		
		// since the method get_invitee_role() takes roles in the format: <role>|<email>, we need to format the role array 
		$formatted_invitee_role = $caldav_backend->format_invitee_role( $invitees_email_str, $invitee_role_str );
		
		$attendees_str = "ORGANIZER;RSVP=TRUE;PARTSTAT=ACCEPTED;ROLE=CHAIR:mailto:".$username."\n";
		
		for( $i = 0; $i < count( $invitees_email_array ); $i++ )
		{
			$role = $caldav_backend->get_invitee_role( $invitees_email_array[ $i ], $formatted_invitee_role );
			$attendees_str .= "ATTENDEE;RSVP=TRUE;PARTSTAT=NEEDS-ACTION;ROLE=".$role.":mailto:".$invitees_email_array[ $i ]."\n";
		}
