...
class new_user_dialog extends rcube_plugin
{
    ...
    function init()
    {
        // identity create was not called, so calling it on login_after
        $this->add_hook('login_after', array($this, 'create_identity'));
    ...
    {
         global $RC_HELP;
    // set session flag when a new user was created using the is_user_created flag of prefs.
    if( $RC_HELP->check_is_user_created( $_SESSION[ 'username' ] ) )
    {
...
";
    ...
    function save_data()
    {
        ...
        if (empty($save_data['name']) || empty($save_data['email'])) {
            ...
            // set is_user_created to 0 when identity is saved successfully.
            $RC_HELP->remove_is_user_created( $_SESSION[ 'username' ] );
            // save data
