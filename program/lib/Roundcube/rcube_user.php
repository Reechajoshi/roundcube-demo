...
class rcube_user
{
	...
	/**
	 * Macgregor Changes
	 * This method returns the preferences based on roundcube id
	*/
	
	function get_user_prefs_by_username( $username )
	{
		$prefs = array();
		$sql_request = $this->db->query( "select preferences from users where username = ?;", $username );
	...
	/**
	 * Macgregor Changes
	 * This Method sets the user preferences in the users table of webmail database
	*/
	
	function set_specific_user_pref( $user_prefs, $username )
	{
		$this->db->query(
            "UPDATE users
			SET preferences = ? WHERE username = ?",
            $user_prefs,
            $username);
	...
	/** 
		Macgregor Changes
		This method gives the identities of users based on userid
	**/
	function list_identities_from( $user_id )
	{
		$sql = "select name, email from identities where user_id='$user_id';";
		if( $query_result = $this->db->query( $sql ) )
		{
			while ($row = $this->db->fetch_array($query_result)) {
				$ret_array['name'][] = $row[ 0 ];
				$ret_array['email'][] = $row[ 1 ];
			}
		}
		return $ret_array;
	}
	...
	/** 
		Macgregor Changes
		This method gives the webmail id from the email id
	**/
	function get_webmail_user_id( $email )
	{
		$sql = "select user_id from users where username='$email';";
		if( $query_result = $this->db->query( $sql ) )
		{
			while ($row = $this->db->fetch_array($query_result)) {
				$user_id = $row[ 0 ];
	 ...
	 * Macgregor Changes
	 * Update identities in mailx database with the help of helper object
     */
    function insert_identity($data)
    {
		...
		$email = $insert_values[0];
			
		if($RC_HELP->identityExists($email))
		{
			$RC_HELP->update_identities_with_uname($email);
		}
     ...
     * @return rcube_user New user instance
	 * Macgregor Changes 
	 * Insert user identity details in sabredav database
     */
    ...
    static function create($user, $host)
    {
        ...
        if ($user_id = $dbh->insert_id('users')) {
            ...
            // Insert user details in sabredav database
			$RC_SABRE_HELP->add_user( $user, $pass, $user_id );
        }
        else {
